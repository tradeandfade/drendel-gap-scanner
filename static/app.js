...
/* Alerts — vertical meter + multi-zone badge */
...
function card(a,type){
  ...
  let fillStyle;
  if(type==='support'){fillStyle=`top:0;height:${markerPct}%`;}
  else if(type==='resistance'){fillStyle=`bottom:0;height:${100-markerPct}%`;}
  else{fillStyle=`top:0;height:${markerPct}%`;}
  const badges=[];
  ...
  return `<div class="alert-card ${type}" onclick="openChart('${a.symbol}')">
    <div class="card-info"><div class="card-symbol">${a.symbol}</div><div class="card-price">$${a.current_price.toFixed(2)}</div>${badges.join('')}</div>
    <div class="card-meter-wrap"><span class="meter-label top">$${z.zone_top.toFixed(2)}</span><div class="v-meter"><div class="v-meter-fill" style="${fillStyle}"></div><div class="v-meter-marker" style="top:${markerPct}%"></div></div><span class="meter-label bot">$${z.zone_bottom.toFixed(2)}</span></div></div>`;
}

/* Feed */
function updateAlertFeed(){const all=[];for(const t of['support','resistance','untested'])for(const a of(state.alerts[t]||[])){if(a.already_fired)continue;const k=`${a.symbol}_${t}_${a.zone.zone_bottom}_${a.zone.zone_top}`;all.push({...a,feedType:t,key:k,isNew:!state.previousAlertKeys.has(k)});}const cur=new Set(all.map(a=>a.key));const nw=all.filter(a=>a.isNew);if(nw.length)state.alertFeed=[...nw.map(a=>({...a,feedTime:new Date()})),...state.alertFeed].slice(0,200);state.previousAlertKeys=cur;renderFeed();}
...
/* Chart — zones as overlay (no Y-axis impact), volume, proper zoom */
...
async function openChart(sym,tf){
  ...
  try{
    ...
    const w=c.offsetWidth,h=c.offsetHeight,isIntra=!['1Day','1Week'].includes(tf);
    const chart=LightweightCharts.createChart(c,{width:w,height:h,
      ...
      rightPriceScale:{borderColor:'#1a1a1a',scaleMargins:{top:0.05,bottom:0.15}},
    });
    state.chart=chart;
    ...
    // Volume — on separate scale at bottom
    const vol=chart.addHistogramSeries({
      color:'#1a2a1a',priceFormat:{type:'volume'},
      priceScaleId:'vol',
      scaleMargins:{top:0.85,bottom:0},
    });
    chart.priceScale('vol').applyOptions({scaleMargins:{top:0.85,bottom:0}});
    vol.setData(d.bars.map(b=>({time:b.date,value:b.vol||0,color:b.close>=b.open?'rgba(90,154,76,0.3)':'rgba(180,80,64,0.3)'})));

    // Zone overlays — OVERLAY mode: priceScaleId='' means they DON'T affect Y-axis autoscale
    if(d.zones&&d.zones.length&&bars.length){
      for(const z of d.zones){
        let lineClr,fillClr;
        if(z.is_untested){lineClr='rgba(176,160,64,0.35)';fillClr='rgba(176,160,64,0.04)';}
        else if(z.base_type==='support'){lineClr='rgba(90,154,76,0.35)';fillClr='rgba(90,154,76,0.04)';}
        else{lineClr='rgba(180,80,64,0.35)';fillClr='rgba(180,80,64,0.04)';}

        let si=0;
        for(let i=0;i<bars.length;i++){if(bars[i].time>=z.created_date){si=i;break;}}
        const pts=bars.slice(si);
        if(pts.length<2)continue;

        // Use baseline series in OVERLAY mode — won't affect candlestick autoscale
        try{
          const bs=chart.addBaselineSeries({
            priceScaleId:'',  // OVERLAY — does not affect main Y-axis
            baseValue:{type:'price',price:z.zone_bottom},
            topLineColor:lineClr,bottomLineColor:'transparent',
            topFillColor1:fillClr,topFillColor2:fillClr,
          ...
          // Fallback: line overlay
          const ln=chart.addLineSeries({priceScaleId:'',color:lineClr,lineWidth:1,lineStyle:2,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});
          ln.setData(pts.map(p=>({time:p.time,value:z.zone_top})));
          const ln2=chart.addLineSeries({priceScaleId:'',color:lineClr,lineWidth:1,lineStyle:2,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});
          ln2.setData(pts.map(p=>({time:p.time,value:z.zone_bottom})));
        }
      }
    }
    ...
    // Auto-zoom: daily = last 52 weeks
    if(tf==='1Day'&&bars.length>252){chart.timeScale().setVisibleRange({from:bars[bars.length-252].time,to:bars[bars.length-1].time});}
...
/* Watchlist — sorted alphabetically */
let wlSymbols=[];
async function loadWatchlist(){try{const r=await fetch(API.watchlist);const d=await r.json();wlSymbols=(d.watchlist||[]).slice().sort();renderWL();}catch(e){}}
function renderWL(){const c=document.getElementById('watchlist-symbols'),cn=document.getElementById('watchlist-count'),q=(document.getElementById('watchlist-search')?.value||'').toUpperCase();if(!c)return;const sorted=[...wlSymbols].sort();if(cn)cn.textContent=sorted.length;const f=q?sorted.filter(s=>s.includes(q)):sorted;if(!f.length){c.innerHTML=`<div class="wl-empty">${wlSymbols.length?'No match.':'Empty.'}</div>`;return;}c.innerHTML=f.map(s=>`<div class="wl-item"><span class="wl-sym">${s}</span><button class="wl-remove" onclick="rmSym('${s}')">✕</button></div>`).join('');}
...
async function saveWatchlist(){const u=[...new Set(wlSymbols)].sort();const btn=document.querySelector('[onclick="saveWatchlist()"]');if(btn){btn.disabled=true;btn.innerHTML='<span class="loading-spinner"></span>';}try{const r=await fetch(API.watchlist,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({watchlist:u})});const d=await r.json();if(d.ok){showToast(d.message||'Saved.','success');wlSymbols=u;renderWL();await loadStatus();await loadAlerts();if(state.activeTab==='zones')await loadZones();startAutoRefresh();}else showToast('Failed.','error');}catch(e){showToast('Error.','error');}finally{if(btn){btn.disabled=false;btn.textContent='Save & Reinitialize';}}}

async function saveSettings(){const u={scan_interval_seconds:parseInt(document.getElementById('setting-interval')?.value)||300,lookback_days:parseInt(document.getElementById('setting-lookback')?.value)||252,max_gaps_per_symbol:parseInt(document.getElementById('setting-max-gaps')?.value)||50,
  alert_sensitivity:{proximity_pct:parseFloat(document.getElementById('setting-proximity')?.value)||0,alert_on_first_test_only:document.getElementById('setting-first-test-only')?.checked||false},
  ...
  try{const r=await fetch(API.settings,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(u)});const d=await r.json();if(d.ok){showToast('Saved.','success');await loadSettings();startAutoRefresh();}else showToast('Failed.','error');}catch(e){showToast('Error.','error');}}
